...
export default class GameScreen extends Screen {
  ...
  reset(options?: { advanceLevel?: boolean }): void {
    ...
    for (let r = 0; r < this.pegRows; r++) {
      for (let c = 0; c < this.pegColumns; c++) {
        ...
        if (r % 2 === 1 && c === this.pegColumns - 1) continue;

        if (this.pegColumns % 2 === 0) {
          if (r % 2 === 1 && c === this.pegColumns / 2 - 1) continue;
        } else {
          if (r % 2 === 0 && c === this.pegColumns / 2 - 1) continue;
        }
  ...
  handleCollision(ball: Ball, actor: Actor): void {
    if (!ball.isActive || !actor.isActive) return;

    if (actor instanceof Paddle) {
      const paddle = actor as Paddle;
